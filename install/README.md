...
**Multiple providers / keys:** In Settings, pick a **provider** (OpenAI, Gemini, …); the **Saved API keys** list shows **only keys for that provider**. You can store **many labeled keys per vendor** (labels must be **unique per provider**). Keys live in **`worker-manager/llm_credentials.json`**. In **Agents → Configure agent models**, use **one row per agent role** (intake, PM, implementer, …): **Vendor → Saved API key → Model**. Each role can use a different labeled key. The worker manager reads **`worker-manager/agent_models.json`** every cycle and updates tasks with that role’s credential (so changes apply on the next manager poll, without restarting workers). **Agent Operations** cards show an **API key** column (the key’s label).
