...
ChatGPT/Codex OAuth: prefer **`./flume codex-oauth login-browser`** (see **OpenAI ChatGPT / Codex OAuth**); device **`login`** may lack `api.responses.write`. When Flume is using OpenAI + OAuth and a Codex session is present, **Plan New Work** can route through **`codex app-server`** automatically instead of the plain HTTP OpenAI path.
