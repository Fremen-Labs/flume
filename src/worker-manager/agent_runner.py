...
def _task_uses_codex_app_server(task: Optional[dict[str, Any]]) -> bool:
    if not task:
        return False
    if not (codex_bridge.codex_auth_present() and codex_bridge.codex_available()):
        return False
    ...
    model = str(task.get('preferred_model') or '').strip().lower()
    if 'codex' in model:
        return True
    ...
    if _task_uses_codex_app_server(task) and repo_path:
        ...
        try:
            ...
            if files:
                ...
                repo_file_hint = (
                    "\n\nREPO FILE CANDIDATES (edit at least one for code tasks):\n"
                    + preview
                )
        except Exception:
            repo_file_hint = ''
