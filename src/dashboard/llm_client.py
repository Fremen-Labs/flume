...
def _openai_bearer_for_request(rt: dict) -> str:
    """
    Return the bearer token for OpenAI requests.

    ChatGPT/Codex OAuth stores the canonical access token in the OAuth state file
    (`.openai-oauth.json`). `LLM_API_KEY` in .env is a synced copy and can lag behind
    after `login-browser` / import — using it first caused 401 (missing scopes) even
    when the file already had a good token.
    """
